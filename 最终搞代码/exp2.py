...
class LinearSys():
    ...
    def assembleLinear(self, c = 1, Ke = np.array([[1,-1],[-1,1]]), u_node= ['u1','u2','u3','u4','u5'], f_b = 0, f_s = 5):
        '''
        Task 6.2: assembly routine of stiffness matrix for linear elements.
        The element stiffness matrix here indicates a linear relationship
        of values between neighbouring elements.
        ...
        print(f'{c} * {K} · {u_node}.T')
        print('--------# right hand side of the equation #--------')
        rhs = np.zeros(n)
        rhs[0], rhs[-1] = f_b, f_s
        print(f'f_b + f_s = {rhs}.T')
    ...
    def postProcess(self):
        ...
        for BC in range(6):
            ...
            grad_1 = np.zeros(6)
            d_element = np.zeros(6)      # local displacement in element
            for i in range(5):
